...
hexo.extend.helper.register('cr_paginator', function (options){
  ...
  if (prevNext){
    result += '<a class="item' + (current > 1 ? '' : ' disabled') + '" rel="prev" href="' + link(current - 1) + '">' + prevText + '</a>';
  }
  ...
  if (options.show_all){
    ...
    var spaceHtml = '<div class="disabled item">' + space + '</div>';
  ...
  if (prevNext){
    result += '<a class="item' + (current < total ? '' : ' disabled') + '" rel="next" href="' + link(current + 1) + '">' + nextText + '</a>';
  }
